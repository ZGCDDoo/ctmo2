    ...
    void CleanUpdate()
    {
        ...
        for (size_t iUp = 0; iUp < kkup; iUp++)
        {
            for (size_t jUp = 0; jUp < kkdown; jUp++)
            {
                ...
                if (iUp == jUp)
                {
                    nfdata_.Nup_(iUp, iUp) -= FAux(FermionSpin_t::Up, dataCT_->vertices_.at(iUp).aux());
                }
            }
        }
        ...
        for (size_t iDown = 0; iDown < kkdown; iDown++)
        {
            for (size_t jDown = 0; jDown < kkdown; jDown++)
            {
                ...
                if (iDown == jDown)
                {
                    nfdata_.Ndown_(iDown, iDown) -= FAux(FermionSpin_t::Down, dataCT_->vertices_.at(iDown).aux());
                }
            }
        }
