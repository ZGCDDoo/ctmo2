...
template <typename TModel, typename TIOModel> class KineticEnergy
{
  public:
    ...
    double GetKineticEnergy()
    {
        ...
        const ClusterMatrixCD_t TM =
            (tLoc + selfEnergyZM - mu * ClusterMatrixCD_t(Nc, Nc).eye()) * (tLoc + selfEnergyZM - mu * ClusterMatrixCD_t(Nc, Nc).eye()) +
            hybFM + selfEnergyFM;
        ...
        for (size_t ii = 0; ii < ioModel_.indepSites().size(); ii++)
        {
            ...
            const double temp = Fourier::MatToTauAnalytic(greenMat_.tube(s1, s2), beta / 2.0, beta, FM(s1, s2).real(), SM(s1, s2).real(),
                                                          TM(s1, s2).real());
            fout << temp << " ";
        }
        fout << std::endl;
        ...
        for (size_t jj = 0; jj < greenMat_.n_cols; jj++)
        {
            ...
            {
                greentau0(ii, jj) =
                    Fourier::MatToTauAnalytic(greenMat_.tube(ii, jj), TAU0, beta, FM(ii, jj).real(), SM(ii, jj).real(), TM(ii, jj).real());
...
#else
        Logging::Warn("Warning, Kinetic Energy **might** be wrong, needs additionnal verification.");
#endif
